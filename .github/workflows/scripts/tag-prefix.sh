...
# path from the repo root. delivery-agent-team-tools' go.mod lives at the
# repo root itself (path "."), so there is no directory segment to require:
# the tag is a bare [v]X.Y.Z.
set -euo pipefail
