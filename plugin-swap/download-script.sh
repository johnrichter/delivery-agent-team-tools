#!/usr/bin/env sh
# download-script.sh -- generic per-OS/arch binary provisioner. A plugin
# never edits this file in place; its SessionStart hook execs it with the
# env below set to that plugin's own CLI name, data dir, and release host.
#
...
# crash): the caller (a plugin's bootstrap hook) treats an unresolved binary
# the same as a CLI that was never installed and fails open or reports it,
# per that plugin's own contract.
#
# Inputs (env):
...
#                        (a GitHub release's own download root already has
#                        the shape this script expects).
